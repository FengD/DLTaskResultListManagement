...
public interface DetecitonTaskMapper {
	...
	@Insert("INSERT INTO crdc_airi_detection_task(name,authors,description,test_dataset_url,"
			...
			+ "result_details_url,state_report_url,nb_object_class,platform,inference_time_second,percision1,"
			+ "recall1,offset_pixel,mse,percision2,recall2) VALUES (" + "#{name,jdbcType=VARCHAR}, "
			...
			+ "#{percision1,jdbcType=FLOAT}," + "#{recall1,jdbcType=FLOAT}," + "#{offset_pixel,jdbcType=FLOAT},"
			+ "#{mse,jdbcType=FLOAT}," + "#{percision2,jdbcType=FLOAT}," + "#{recall2,jdbcType=FLOAT}" + ")")
	int insert(String name, String authors, String description, String test_dataset_url, String model_files_url,
			...
			float mse, float percision2, float recall2);
